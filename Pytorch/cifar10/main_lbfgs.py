...
parser.add_argument('--memory', '-m', default=20, type=int, help='memory size for lbfgs')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    optimizer = optim.LBFGS(net.parameters(), lr=args.lr, max_iter=20, max_eval=None, tolerance_grad=1e-07, tolerance_change=1e-09, history_size=args.memory, line_search_fn=None)
    ...
    def train(epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            for p in net.parameters():
                l1 = l1 + (p.abs()**2).sum()
            loss = loss + args.weight_decay * l1
            optimizer.zero_grad()
